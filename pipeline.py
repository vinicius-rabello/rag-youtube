from youtube_parser import get_transcript, parse_transcript
from embedder import generate_embeddings
from prompt_builder import build_prompt_from_file
from retriever import MetadataFAISSRetriever
...
def process_youtube_video(video_id):
    ...
    embedding_vectors = generate_embeddings(chunks, "text-embedding-3-small")
    ...
    return index, chunks, embedding_dim

def process_query(query, index, chunks, embedding_dim, model_name):
    # index = faiss.read_index("resources/index")
    embedding_model = OpenAIEmbeddings(model=model_name)

    retriever = MetadataFAISSRetriever(index, embedding_model, embedding_dim, chunks)
    # query = "O que e quando é falado sobre Cloud nesse vídeo?"
